...
st.markdown("""
    <style>
        /* Tamaño base general (aplica a casi todo el texto de la app) */
        html, body, [class*="css"] {
        ...
        /* Texto general de markdown y párrafos */
        ...
        .stButton>button, .stDownloadButton>button {
            ...
            background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); transition: all 0.3s ease;
            font-size: 22px !important;
        }
        ...
        /* --- Separadores --- */
        hr { border: none; height: 2px; background: linear-gradient(90deg, transparent, #007bff, transparent); margin: 30px 0; }
        ...
        /* --- Alertas: error, info, warning, success --- */
        div[data-testid="stAlert"], div[data-testid="stAlert"] p {
            font-size: 22px !important;
        }

        /* --- Bloques de código (logs) --- */
...
# --- FUNCIONES AUXILIARES (Refactorización para evitar IFs) ---
def validar_url(url):
    try:
...
def normalizar_datos(df):
    """Garantiza que el DF tenga la estructura esperada, evitando ifs posteriores."""
    columnas_base = {'Titulo': '', 'Precio': pd.NA, 'Categoria': 'Desconocida', 'Descripcion': '', 'URL': ''}
...
def extraer_palabras_comunes(serie_texto, top_n=15, nombre_columna='Palabra'):
    """Extrae palabras clave y retorna un DF listo para mostrar, vacío si no hay datos."""
    ...
    return pd.DataFrame(Counter(palabras).most_common(top_n), columns=[nombre_columna, 'Frecuencia'])
...
with st.sidebar:
    ...
    url_input = st.text_input("", placeholder="https://ejemplo.com/empleos", label_visibility="collapsed")
...
if btn_start:
    if not url_input or not validar_url(url_input):
        ...
        with st.status("⏳ Analizando sitio web...", expanded=True) as status:
            ...
            try:
                df_datos = CrawlerEmpleo(url_input).recorrer(max_paginas=max_paginas, callback=update_logs)
                if df_datos.empty:
...
if not df.empty:
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Resumen", "📈 Análisis de Texto", "🔍 Explorador Interactivo", "💾 Exportar"])

    # Preparación de datos limpios para uso general
    ...
    with tab1:
        ...
        # Métricas simplificadas con operadores ternarios
        ...
        with c1:
            st.markdown("<p style='font-size: 26px; font-weight: 600; color: #2c3e50;'>🏢 Distribución por Categoría</p>", unsafe_allow_html=True)
            cat_counts = df['Categoria'].value_counts().head(10).reset_index()
            cat_counts.columns = ['Categoría', 'Cantidad']
            fig1 = px.bar(cat_counts, x='Cantidad', y='Categoría', orientation='h', color='Cantidad', text='Cantidad')
            fig1.update_layout(yaxis={'categoryorder': 'total ascending'}, showlegend=False, plot_bgcolor='rgba(0,0,0,0)', font=dict(size=18))
            st.plotly_chart(fig1, use_container_width=True)

        with c2:
            st.markdown("<p style='font-size: 26px; font-weight: 600; color: #2c3e50;'>💰 Distribución Salarial</p>", unsafe_allow_html=True)
            if hay_precios:
                fig2 = px.histogram(df_precios, x="Precio", nbins=20, color_discrete_sequence=['#28a745'])
                fig2.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(size=18))
    ...
    with tab2:
        ...
        with col1:
            st.markdown("<p style='font-size: 26px; font-weight: 600; color: #2c3e50;'>📌 Palabras más usadas en Títulos</p>", unsafe_allow_html=True)
            df_palabras = extraer_palabras_comunes(df['Titulo'], nombre_columna='Palabra')
            if not df_palabras.empty:
                fig_words = px.bar(df_palabras, x='Frecuencia', y='Palabra', orientation='h', color_discrete_sequence=['#6f42c1'])
                fig_words.update_layout(yaxis={'categoryorder': 'total ascending'}, font=dict(size=18))
                st.plotly_chart(fig_words, use_container_width=True)
            else:
                st.info("📭 Sin datos de títulos suficientes.")

        with col2:
            st.markdown("<p style='font-size: 26px; font-weight: 600; color: #2c3e50;'>📝 Habilidades Requeridas (En Descripción)</p>", unsafe_allow_html=True)
            df_habs = extraer_palabras_comunes(df['Descripcion'], top_n=12, nombre_columna='Habilidad')
            st.dataframe(df_habs, use_container_width=True, hide_index=True)
    ...
    with tab3:
        ...
        with f1:
            st.markdown("<p style='font-size: 24px; font-weight: 600;'>🔎 Buscar palabra clave</p>", unsafe_allow_html=True)
            busqueda = st.text_input("", label_visibility="collapsed", key="busqueda_input")
        with f2:
            st.markdown("<p style='font-size: 24px; font-weight: 600;'>📂 Categoría</p>", unsafe_allow_html=True)
            categoria_filter = st.multiselect("", options=df['Categoria'].dropna().unique(), label_visibility="collapsed")

        # VALIDACIÓN DEL SLIDER INTEGRADA Y LIMPIA
        with f3:
            min_p = int(df_precios['Precio'].min()) if hay_precios else 0
            max_p = int(df_precios['Precio'].max()) if hay_precios else 0

            if not hay_precios:
                st.markdown("<p style='font-size: 22px;'>📭 Ofertas sin sueldo definido</p>", unsafe_allow_html=True)
                price_range = (0, 0)
            elif min_p == max_p:
                st.markdown(f"<p style='font-size: 22px;'>💡 Salario único: ${min_p}</p>", unsafe_allow_html=True)
                price_range = (min_p, max_p)
            else:
                st.markdown("<p style='font-size: 24px; font-weight: 600;'>💰 Rango Salarial</p>", unsafe_allow_html=True)
                price_range = st.slider("", min_value=min_p, max_value=max_p, value=(min_p, max_p), label_visibility="collapsed")

        # Aplicación de filtros encadenados (mucho más limpio)
        df_filtrado = df.copy()
        if busqueda:
        ...
        if hay_precios:
            df_filtrado = df_filtrado[df_filtrado['Precio'].between(price_range[0], price_range[1]) | df_filtrado['Precio'].isna()]

        st.markdown(f"<p style='font-size: 24px; font-weight: 600;'>📋 Mostrando {len(df_filtrado)} resultados</p>", unsafe_allow_html=True)
    ...
    with tab4:
        ...
        with e1:
            st.download_button("📥 CSV", df.to_csv(index=False).encode('utf-8'), "analisis.csv", "text/csv", use_container_width=True)
        with e2:
            st.download_button("📥 JSON", df.to_json(orient='records', indent=2).encode('utf-8'), "analisis.json", "application/json", use_container_width=True)
        with e3:
            try:
                ...
                with open("temp.xlsx", "rb") as f:
                    st.download_button("📥 Excel", f.read(), "analisis.xlsx", "application/vnd.ms-excel", use_container_width=True)
            except ImportError:
                st.button("📥 Excel no disponible", disabled=True, use_container_width=True)

else:
    # ESTADO VACÍO
    st.markdown("<h2 style='text-align: center; color: #667eea; padding: 40px; font-size: 56px;'>👋 Ingresa una URL para comenzar</h2>", unsafe_allow_html=True)